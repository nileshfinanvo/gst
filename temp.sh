...
#mysql --user=root --password=India@123 -e "CREATE DATABASE mca /*\!40100 DEFAULT CHARACTER SET utf8 */;"
#mysql -uroot -p${rootpasswd} -e "CREATE USER admin@'%' IDENTIFIED BY 'India@123';"
#mysql --user=root --password=India@123 -e "CREATE USER admin@'%' IDENTIFIED BY 'India@123';"
#mysql -uroot -p${rootpasswd} -e "GRANT ALL PRIVILEGES ON *.* TO 'admin'@'%';"
#mysql --user=root --password=India@123 -e "GRANT ALL PRIVILEGES ON *.* TO 'admin'@'%';"
#mysql -uroot -p${rootpasswd} -e "FLUSH PRIVILEGES;"
#mysql --user=root --password=India@123 -e "FLUSH PRIVILEGES;"

echo "Enter Github Token"
#git clone https://github.com/MCA-Download/MCADB.git
git clone https://nileshfinavo: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@github.com/nileshfinanvo/gst_modified.git
echo "Enter root MYSQL password"
#mysql -u root -p mca < /home/ubuntu/MCADB/mca.sql
mysql --user=root --password=India@123 mca < /home/ubuntu/MCADB/mca.sql

echo "Installing Dependencies"

sudo apt install -y python3-pip
pip3 install PyPDF2 PyMySQL boto3 lxml xlwt xlrd bs4
#chmod 644 /home/ubuntu/MCADB/50-cloud-init.yaml
#sudo mv /home/ubuntu/MCADB/50-cloud-init.yaml /etc/netplan/

cron_line="@reboot sh /home/ubuntu/MCADB/server.sh"
cron_line="@reboot sh /home/ubuntu/MCADB/server.sh"
(crontab -u ubuntu -l; echo "$cron_line" ) | crontab -u ubuntu -

#sudo mv /home/ubuntu/MCADB/mysqld.cnf /etc/mysql/mysql.conf.d/
#aws configure
#aws configure set aws_access_key_id AKIATTT5CTDTUPHUSYBW; aws configure set aws_secret_access_key 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; aws configure set default.region us-east-2
chmod +x MCADB/server.sh
#nano +94,11 MCADB/read_write_database.py
